...
# imageio (2.10.5), matplotlib (3.9.2), numpy (1.26.4), Pillow (9.4.0), scipy (1.13.1)
...
import matplotlib as mpl # <= 3.9
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import Divider, Size
from PIL import ImageTk, Image
from scipy.linalg import expm, norm

GUI_NAME = "VNS_Plattform"
GUI_VERSION = '1.7'
...
class NewGUI():
    def __init__(self):
        ...
        if not os.path.isfile(GUI_NAME + "_Skizze.png"):
            print("Skizze nicht gefunden!! ... Fahre fort")
        else:
            pix_per_pts = self.root.winfo_fpixels('1p')
            print("%.2f" % pix_per_pts + " pixels per point")
            img = ImageTk.PhotoImage(Image.open(GUI_NAME + "_Skizze.png").resize((int(160*pix_per_pts),int(240*pix_per_pts)), Image.LANCZOS))
    ...
    def save(self):
        try:
            ...
            tk.messagebox.showinfo('Finished', 'Finished export.')
            print("finished.")
        except Exception as e:
            print(e)
            tk.messagebox.showerror('Error', repr(e))
